class Solution {
    public int networkDelayTime(int[][] times, int n, int k) {
        Map<List<Integer>, Boolean> visitedMap = new HashMap<>();
        ...
        dfs(times, n, k, 0, visitedMap, timeMap);
    ...
    public int dfs(int[][] times, int n, int k, int currentSum, Map<List<Integer>, Boolean> visitedMap, Map<Integer, Integer> timeMap) {
        for(int i = 0; i < times.length; i++) {
            if(times[i][0] == k) {
                ...
                if(currentSum + weight < timeMap.get(nextNode)) {
                    ...
                    dfs(times, n, nextNode, currentSum + weight, visitedMap, timeMap);
                }
            }
        }
